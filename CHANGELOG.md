...
### Fixed
- On EE multishop, subshops no longer share the base shop's API-user setup token. Module activation generated the setup token only when no token value was present, but a freshly created subshop inherits the base shop's module settings (including that token), so every subshop reused the base shop's token, which is the single gate on the unauthenticated `heartbeatSetPassword` mutation. Activation now generates a fresh per-shop token whenever the current shop's service-user password is not yet set, and only skips generation once that shop's setup is complete (bcrypt password on its own row). (OXS-3103)
...
- Widened the dependency constraints so the module can be installed on OXID eShop 7.5: `graphql-base` now also allows `^13.0` and `graphql-configuration-access` now also allows `^4.0`, the versions OXID 7.5 ships, in addition to the existing ranges covering 7.1 to 7.4. `graphql-configuration-access` v4 keeps the same settings interface, so no code change was required. Without this the published constraints excluded the 7.5 GraphQL stack, so `composer require` on a 7.5 shop could not select this line and fell back to an ancient tag. (OXS-3127)
