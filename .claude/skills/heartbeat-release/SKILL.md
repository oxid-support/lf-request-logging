...
* **MAJOR**: any BC break. This includes removing/renaming a released GraphQL operation, changing an operation's arguments, or a PHP-level BC break (removed/renamed public method, class, or interface member). A MAJOR does **not** take "line-major + 1"; it takes the **next free major across ALL lines** (`git tag --sort=-v:refname` for the highest used) and gets a **codename**.

Determine BC honestly: `git tag --contains <commit>` tells you whether something you removed was ever released. Removing a released operation is a MAJOR even if no known client used it. Do not blindly tag the current `VERSION` constant, it may be a stale pre-planned bump that no longer reflects the accumulated `[Unreleased]` scope; reassess against everything under `[Unreleased]`.
