...
4. **Update each line's `CHANGELOG.md`** under `## [Unreleased]` (Keep a Changelog format, English). Each line has its own changelog file describing its own history; never copy an entry to a line that did not receive the change. When removing something, check whether it shipped: `git tag --contains <introducing-commit>`. If a released tag contains it, it is a breaking change and needs a `Removed` entry; if it is still unreleased, just drop its `Unreleased` line (net never-shipped). `composer.lock` is gitignored, so never commit it; and do not `git add -A` blindly, PHPUnit cache dirs (`tests/.phpunit*.cache`) can sneak in. Lead every entry with one short, plain-language summary sentence (what changed), then the why/detail in the same entry; a reader scanning `[Unreleased]` should grasp each change from its first sentence.
