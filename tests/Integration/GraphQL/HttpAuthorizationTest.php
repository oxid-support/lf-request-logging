 ...
 * web server, deliberately not in-process (OXID 6.5 line).
 *
 ...
 *
 * Unlike the 7.x lines this does not extend graphql-base's TokenTestCase
 * (graphql-base v7 test bases do not load on the phpunit 8.5 platform of
 * OXID 6.5); it is a plain TestCase that only talks HTTP.
 *
 ...
 * A skip here means: HTTP auth is NOT verified.
 */
final class HttpAuthorizationTest extends TestCase
{
    ...
    {
        // Generous timeout on purpose: a cold shop recompiles caches on the
        // first request. A slow warm-up must not be mistaken for "web server
        // down" (a down stack refuses the connection fast).
    ...
    /**
     * The token query issues real rows in oegraphqltoken outside any test
