...
// graphql-base ships its integration TestCase base classes (TokenTestCase etc.)
// under autoload-dev, so they are not on the autoloader when graphql-base is a
// dependency. v8 (OXID 7.0) still exposed them via autoload; v9+ (OXID 7.1+) does
// not. Register them from the installed package so the harness works on every line.
if (class_exists(\Composer\InstalledVersions::class)) {
    $graphqlBasePath = \Composer\InstalledVersions::getInstallPath('oxid-esales/graphql-base');

    if ($graphqlBasePath !== null) {
        $graphqlBaseTests = $graphqlBasePath . '/tests';

        spl_autoload_register(static function (string $class) use ($graphqlBaseTests): void {
            $prefix = 'OxidEsales\\GraphQL\\Base\\Tests\\';
            if (strncmp($class, $prefix, strlen($prefix)) !== 0) {
                return;
            }

            $relative = str_replace('\\', '/', substr($class, strlen($prefix))) . '.php';
            $file = $graphqlBaseTests . '/' . $relative;
